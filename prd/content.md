...
**目录** ：
  - 预览单页面时，不显示目录，同线上；
  - 预览多页面时，显示目录区域，同线上；
  - 预览生成的应用时，显示左侧目录区域，支持通过拖拽右侧分割线调整宽度（同代码模式下的宽度调整交互）；
  - **非记忆性逻辑**：用户调整目录宽度后，若切换至其他 Tab（如代码或编辑 Tab）再返回，目录宽度将恢复为系统的默认宽度，不保留用户的个性化调整。
...
**目录** ：
  - 编辑单页面时，不显示目录，同线上；
  - 编辑多页面时，显示目录区域，同线上；
  - 编辑生成的应用时，显示左侧目录区域，支持通过拖拽右侧分割线调整宽度（同代码模式下的宽度调整交互）；
  - **非记忆性逻辑**：用户调整目录宽度后，若切换至其他 Tab（如预览或代码 Tab）再返回，目录宽度将恢复为系统的默认宽度，不保留用户的个性化调整。
