...
- **1. 发布前（未发布态）**
  - **弹框内容**：显示项目名称、当前状态、访问链接，底部显示全宽"发布"按钮。
  - **当前状态**：显示灰色"未发布"胶囊。
  - **项目名称（始终可编辑）**：
    - **默认与限制**：默认填充当前会话标题。长度限制 1-50 个字符；编辑时不可超 50 字符；非编辑状态超长则尾部省略号 `...` 截断。
    - **格式支持**：支持任意语言、符号、数字组合。
    - **校验规则**：光标移出输入框时校验敏感词，违规则输入框下方飘红提示"包含敏感词汇，请修改"；项目名称在任意发布状态下均可编辑修改。
  - **访问链接**：
    - **域名格式**：限制使用墨刀三级域名 `{slug}.modao.site`，不支持自定义域名。
    - **前缀规则**：仅限小写字母（a-z）、数字（0-9）和中划线（-），中划线不可首尾；长度 3-30 字符；输入时实时过滤非法字符，输入框下方显示格式提示"仅支持小写字母、数字和中划线"。
    - **未发布状态交互**：Hover 显示下划线，点击链接新标签页打开，并显示提示"该页面尚未发布，当前为预览模式"。
    - **已发布状态交互**：Hover 显示下划线，点击链接新标签页打开正式公开页面。
    - **编辑态（仅未发布时可编辑 Slug）**：点击编辑图标后图标变为"保存"和"取消"按钮；发布后 Slug 禁止编辑（编辑按钮置灰），若需修改须先撤回再编辑，修改后重新发布，旧链接立即失效（返回 404）。
    - **校验规则**：光标移出时校验；含敏感词报错"该链接包含违规字符"；域名已被占用报错"该链接已被占用"。

---

- **2. 初次发布操作**
  - **按钮显示**：面板底部仅显示全宽深色"**发布**"按钮。
  - **交互过程（成功路径）**：
    1. 点击"发布" → 按钮进入 Loading 状态（文本变为"正在发布..."，带旋转图标）。
    2. 发布成功 → 按钮立即变为绿色"✅ 发布成功"，同时弹出 Toast："✅ 发布成功！网站已上线"；面板"当前状态"变为绿色"已发布"徽章；新增"上次发布时间"字段（显示"刚刚 · V1"）。
    3. 1s 后，"发布成功"按钮平滑淡入淡出过渡为"**撤回**"与"**更新**"两个并排按钮（面板**不关闭**）；"更新"按钮根据当前内容与快照差异动态决定启用/禁用态。
  - **交互过程（异常路径）**：发布失败（网络或服务器异常）→ 按钮恢复为激活态"发布"，弹出 Toast："网络错误，发布失败，请重试"。
  - **底层逻辑**：生成当前内容快照，初始版本号设为 V1。

---

- **3. 发布后（已发布态）**
  - **弹框内容**：再次点开发布面板，显示项目名称、当前状态（已发布）、上次发布时间、访问链接，底部为"撤回"与"更新"按钮。
  - **当前状态**：显示绿色"已发布"徽章。
  - **上次发布时间**：格式为"X 天前 · Vn"（版本号从 1 开始递增）。
  - **访问链接**：Slug 编辑按钮置灰，发布后不可编辑。

---

- **4. 更新操作**
  - **无变更时**：若 `projectName` 及生成内容与发布快照完全一致，"更新"按钮呈禁用态灰色（`cursor-not-allowed`）。
  - **有变更时**："更新"按钮为激活态深色。
  - **交互过程（成功路径）**：
    1. 点击"更新" → 按钮进入 Loading 状态（"正在更新..."，带旋转图标）。
    2. 更新成功 → 按钮短暂变为绿色"✅ 更新成功"，同时弹出 Toast："✅ 更新成功！"；版本号递增（Vn → Vn+1），"上次发布时间"更新为当前时间。
    3. 1s 后，"更新成功"按钮平滑过渡回"**更新**"，并重新对比当前内容与最新快照：有变更则激活，无变更则禁用。
  - **交互过程（异常路径）**：更新失败 → 按钮恢复激活态"更新"，弹出 Toast："网络错误，更新失败，请重试"。

---

- **5. 撤回操作**
  - **按钮显示**：已发布状态下始终显示"**撤回**"按钮。
  - **交互过程（成功路径）**：
    1. 点击"撤回" → 弹出 Ant Design 规范二次确认弹框（标题："确认撤回发布？"，说明："撤回后，网站将无法通过链接访问。"，按钮："取消" / "确定"）。
    2. 用户点击"确定" → "撤回"按钮进入 Loading 状态（"正在撤回..."，带旋转图标）。
    3. 撤回成功 → 弹出 Toast："撤回成功"。面板同步更新：
       - "当前状态"变回灰色"未发布"胶囊；
       - "上次发布时间"字段保留，视觉降级（灰色小字），文案前缀标注"上次发布："，与"已发布"状态区分；
       - 底部按钮合并为全宽"发布"按钮（面板**不关闭**）；
       - 访问链接仍显示，点击时新标签页打开并显示 404 提示；Slug 编辑按钮重新激活可编辑；
       - 线上链接立即失效。
  - **交互过程（异常路径）**：撤回失败 → 弹框关闭，按钮恢复正常态，弹出 Toast："网络错误，撤回失败，请重试"。
