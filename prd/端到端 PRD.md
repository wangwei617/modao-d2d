...
| 版本号 | 修改日期 | 修改人 | 修改内容摘要 |
| --- | --- | --- | --- |
| V1.0 | 2026-03-18 | ww | 初版，按 PRD 规范模板全面梳理 |
| V1.1 | 2026-03-19 | ww | 1. 规范墨刀AI首页推荐和生成应用 Tab 下二级入口命名为「AI生成应用 / AI生成Web应用 / AI生成App」，并优化对应交互与英文提示词。<br>2. 调整「导出至墨刀」下拉菜单顺序，将导出至设计置顶，导出至原型置灰（本期不做）。<br>3. 墨刀原型编辑区选中对象/未选中对象时增加“生成应用”能力。<br>4. 更新私有化场景关于 AI 生成应用入口的说明。<br>5. 其他体验优化，如分享文案优化等。 |
| V1.2 | 2026-03-20 | ww | 1. 支持导出至墨刀原型。<br>2. 取消发布/更新过程中的审核态。<br>3. 作品被撤回/被下架后，「当前状态」展示为 **已下架**。<br>4. 新标签打开已发布访问页可交互预览，被下架/撤回后显示产品已下架占位页。<br>5. 访客页右下角增加与线上一致的 **生成同款**（胶囊按钮）+ **?**（帮助说明）。<br>6. 原型编辑区生成的应用支持以 **AI 组件** 的格式插入到画布。 |
| V1.4 | 2026-03-27 | ww | 1. 删除对话（项目）弹窗文案调整<br>2. 违规被后台下架：同对话内全部生成版本的发布面板中主发布操作置灰 |
| V1.5 | 2026-04-13 | ww | 1. 在积分消耗下拉菜单中补充说明「AI生成应用 按实际消耗计费」。<br>2. 生成应用时，审核用户输入的内容，复用现有的 AI 审核机制。<br>3. 同一对话中不能混着生成 React 与 HTML 文件。<br>4. 设备默认规则：单页/多页 HTML 默认设备均为 PC，不依赖 AI 判断，用户可手动切换。<br>5. 配置清空策略：从 Web/App 切换至通用模式时，清除设计系统与主题色选择，恢复默认。<br>6. 导出降级方案：本期 React 应用导出至墨刀仅支持「可编辑元素」，暂不实现 AI 组件化。<br>7. 自动插入暂停：生成完成后不自动插入画布，避免多页面铺开造成视觉干扰。 |
...
1. **首页输入工作区**：增加生成应用一级类别&二级类别，推荐下增加生成应用入口，选中生成应用时输入框增加设置设计系统和主题色；
2. **墨刀AI生成页**：选中文档/单页html/多页html/应用后顶栏布局调整，分享功能合并，预览/编辑/代码三个tab 下的布局调整；
3. **截图优化工具**：截图入口、标注编辑页、插入对话；
4. **发布与打开链接**：发布弹框（发布、更新和撤回）、访客页面（发布态/下架态）
5. **墨刀工作台&编辑区**：墨刀工作台原型tab下的快速开始模块增加生成应用入口，墨刀原型ai输入框支持生成应用，编辑区内支持生成应用/插入应用
...
| **画幅切片** | 截图模式下，用户在预览区框选的特定坐标区域图像 |
| **三级域名** | 发布时配置的形如 `modao.cc/app/xxx` 的唯一标识 |
...
- **墨刀 D2D 工作区**
  - **首页**
    ...
    - AI能力（分类展示，选中后输入框显示对应标签或新tab打开）：
      - **推荐**：AI生成应用、AI生成原型、图片转原型、需求文档PRD、HTML转原型、生成图片
      - **生成应用**：AI生成应用、AI生成Web应用、AI生成App应用
      - **原型设计**：AI生成原型、图片转原型、HTML转原型、生成原型概念图、功能交互文档、UI规范评审
      - **产品策划**：需求文档PRD、竞品分析、用户调研、产品方案评审、产品规划
      - **PPT**：AI生成PPT、AI美化PPT
      - **图表与图片**：生成图片、流程图、思维导图、用户旅程图
      - **研发测试**：测试用例生成、技术方案生成、技术方案评估
      - **营销与多媒体**：生成视频、生成营销图、SEO文章生成
  - **对话生成区**
    - **左侧对话区**
      - 顶部栏（标题、积分）、对话消息流、底部输入框
    - **右侧生成的html/应用展示区**
      - 预览tab
        - 目录（仅多html显示）、截图优化、页面地址栏、下载、应用界面内容预览
      - 编辑tab
        - 目录（仅多html显示）、截图优化、页面地址栏、撤销/重做、下载、可视化编辑
      - 代码tab
        - 目录（多html和应用均显示）、页面名称、复制、下载、代码编译区
      - 通用能力
        - 导出至墨刀、分享（分享完整项目、分享当前预览）、
        - 发布（发布配置弹窗）
        - 版本切换（V1/V2下拉切换）
        - 窗口控制（全屏、关闭）
...
- **基础态**：输入框的样式、文本输入与文件上传交互，完全**复用线上现有逻辑**。
- **推荐 Tab 更新**：前排新增【生成 Web 应用】推荐入口。
...
- **专有配置工具弹出**：输入框左下角显示设计系统和主题色配置栏（紧跟于“极速/深度”算力切换按钮之后）：
  ...
  2. **主题色智能配置 (Theme & Color)**：
     ...
     - **若选 `Shadcn UI` 等系统级框架**：下拉提供成套的内置主题预设，包含 `自动` 以及 `蓝（#0F63FF）`、`紫（#7856FF）`、`粉（#FF75E8）`、`红（#FF4568）`、`黄（#FFD635）`和`绿（#23C343）` 等颜色主题。
     - **若选 `Ant Design` 等组件化框架**：下拉菜单将纵向裂变为上下两栏布局：上半区为单独的外观开关胶囊（亮色 Light / 暗色 Dark）；下半区为纯色系统主色调选择器（颜色同上）。
- **生成web和生成app的差异**：
  - 选中生成web应用和选中生成app应用后，均走生成应用流程，根据提示词不同生成对应类型的结果，且在生成的预览模式下，默认选中对应的演示设备
  - 选择生成web和生成app时，用的设计系统是同一套


##### 3）其他
- 在上述复杂生成模式激活的状态下，输入区域内的**通用功能交互不变**（包括：直接将本地图片或截图拖拽上传、点击附件 Icon 唤起系统层选取和发送等）
- 通用模式下，根据用户输入的需求判断是否为生成web应用/生成App应用，若为生成应用，则走对应的生成应用流程，设计系统和风格均为默认
...
- **权限**：同线上
- **模块内容**：采用左右分栏布局：左侧为历史对话导航栏，中间为对话消息区（对话标题栏 + 对话消息流 + 输入框），右侧为执行过程/单页html/多页html/应用/文档展示区（预览/编辑/代码三大 Tab + 顶部工具栏）。
...
- **删除对话（项目）弹框内容调整**
  - **弹窗内容**：
    - **标题**：删除项目。
    - **说明**：第一行——「确定要删除项目吗？删除后，已发布的应用和分享链接将一并失效」；第二行——「此操作不可恢复。」
    - **英文（示例）**：*Are you sure you want to delete this project? Published apps and sharing links will stop working.* / *This action cannot be undone.*
...
##### 4）生成逻辑
- 同一对话中只能生成HTML或React应用，两个不可混着生成


##### 5）内容审核
- 在用户发送内容后，增加输入内容的审核逻辑，复用现有的AI输入审核机制

#### 3-2-4、右侧生成的单页/多页/应用展示区
...
在对话消息区生成应用后，默认选中最新生成的应用。右侧预览tab下显示应用的界面内容，界面可点击交互。预览tab下的功能从左向右依次是目录（仅多页html显示）和截图优化、地址栏、复制
- 生成的应用前端为react框架，生成html单页面和html多页面的代码格式保持html现状
...
**地址栏操作区**：
  ...
  - *切换设备*
    - 单击切换设备，下拉显示设备列表，用户可以在PC端、移动端和Pad端之间切换（列表从上到下）。
    - 切换设备的默认项根据用户的生成的结果自动判断，不确定时默认选中 PC端；
      - 选中PC端时，下方生成的界面显示区域在宽度、高度上均撑满显示区域
      - 选中Pad端时，下方生成的界面最大区域为820px宽，1180px高；右侧视口区域小于界面的最大显示区域时，自适应撑满对应的宽/高
      - 选中移动端时，下方生成的界面最大区域为393px宽，852px高；右侧视口区域小于界面的最大显示区域时，自适应撑满对应的宽/高
    - 切换设备时，界面内容根据显示区域尺寸变化而自适应。切换到移动端、Pad端时，界面宽高之外的区域显示背景色，与界面之内的颜色做区分。
   ![alt text](../assets/images/image-21.png)

  - *地址栏*
    - 地址栏中部显示当前正在预览的页面的名称（如 `/home/cart...`），同目录中的页面名称；每次生成后页面默认显示首页的名称
    - 地址栏内的页面名称按照像素宽度截断（显示10个字英文字符宽度），超出10个字符后显示...，
...
**下载**：
  ...
  - 位置从顶栏下移一级
    - 预览生成的单页面时，hover到下载代码按钮时，无下拉菜单，下载功能同线上
    - 预览生成的多页面时，hover到下载代码按钮时，显示下拉菜单，下载功能同线上
    - 预览生成的应用时，下载按钮没有下拉菜单，点击下载按钮，可将整个项目的源代码全部打包下载，zip包形式，功能同线上多页面的下载所有文件的功能
...
**下载**：
  - hover到下载按钮时，显示tooltip：下载代码
  - 位置从顶栏下移一级
    - 编辑生成的单页面时，hover到下载代码按钮时，无下拉菜单，下载功能同线上
    - 编辑生成的多页面时，hover到下载代码按钮时，显示下拉菜单，下载功能同线上
    - 编辑生成的应用时，下载按钮没有下拉菜单，点击下载按钮，可将整个项目的源代码全部打包下载，zip包形式，功能同线上多页面的下载所有文件的功能
...
**下载**：
  - hover到下载按钮时，显示tooltip：下载代码
  - 选中生成的单页面进入代码模式，hover到下载代码按钮时，无下拉菜单，下载功能同线上
  - 选中生成的多页面进入代码模式，hover到下载代码按钮时，显示下拉菜单，下载功能同线上。
  - 选中生成的应用进入代码模式，hover到下载按钮显示下拉菜单，下拉菜单选项为下载当前文件和下载所有文件（下拉菜单内容同多页的下载）
    - 点击下载当前文件，只现在当前所查看的代码
    - 点击下载所有文件，可将整个项目的源代码全部打包下载，zip包形式，功能同线上多页面打包下载

**复制**：
  - hover到复制按钮时，显示tooltip：复制代码
  - 查看单页、多页的代码时，复制功能同线上。
  - 查看应用到代码时，单击复制按钮，可将当前所打开的文件页面内的代码全部复制到剪贴板，功能同线上单页/多页的代码复制，复制成功后弹toast提示“代码复制成功”
  ...
  - **视觉样式与极致缩进处理**：
    ...
    - **宽度调整策略**：
      ...
      3. **尺寸限制**：
         ...
         - **最大宽度**：限制在 400px，确保右侧代码编辑区有足够的操作空间。
    ...
    - **记忆逻辑**：切换预览、编辑tab后再切换回代码tab，在代码tab下依旧选中之前所选中的页面
    - 鼠标 hover 列表项时，单行显示浅色背景，提升视觉反馈。

**标题**：
  - 选中生成的单页、多页、应用时，在目录栏右侧显示当前所选代码页面的标题
...
- **全屏模式交互**：
  ...
  - **显示页面**：全屏内展示的页面与预览/编辑 Tab 下所显示的页面完全一致，背景为深色遮罩，内容区域不保留设备外壳，采用 Fluid 布局填充视口，以确保最大标注空间。
...
- **框选交互**：
  - 在内容区鼠标按下并拖拽，即可绘制矩形选框；选框宽高小于 10px×10px 时不生效。框选区域不受限制，可以与已框选的区域重叠
  ...
  - **空白处点击行为**：
    ...
    - 若当前有编辑中的选框且**有内容**：点击画布空白处，触发选框水平抖动动画（0.5s），提醒用户先确认或取消。
...
- **已确认选框的交互**：
  ...
  - **数字徽章计数**：删除某个标注后，其他评论的数字重新排序

- **选框拖动**：鼠标悬停在未确认/已确认的选框的框体中心时，光标切换为抓手（`grab`）；按住后可自由拖动整个选框位置。

- **选框调整大小**：未确认选框的右下角显示缩放箭头手柄（`nwse-resize`）；按住并拖动可等比/非等比调整选框的宽高，最小宽度 10px，最小高度 10px。

- **技术规格与异常处理**：
  - **截取预览区**：通过 `html2canvas` 抓取。若遇跨域限制导致 iframe 白屏，Toast 提示："无法截取当前内容，请使用系统截图工具进行截图"。
...
- 单页/多页的导出至墨刀功能同线上。
- 生成应用后，单击导出至墨刀，下拉显示菜单，分别为导出至墨刀设计和导出至墨刀原型
  - 导出至墨刀设计： 
    - 单击导出至墨刀设计，弹导出至墨刀弹框，只显示设计文件，可以将生成的应用导出为新文件，或导出至所选的文件（弹框内的交互同原型）
    - 导入到墨刀设计后，在画布内按照独立页面的形式排布（可复用paico的导出至pixso的视觉保真度能力）
  - 导出至墨刀原型：
    - 支持将 React 框架生成的应用，作为一个ai组件导入到原型编辑区，功能同多html页面导入至墨刀
...
- **分享粒度**：面板顶部提供两个标签页切换——"分享完整项目"（全部文件）和"分享当前预览"（当前单文件），默认选中"分享完整项目"。切换时下方显示对应说明文字（“包含整个对话记录和所有生成的内容” / “仅分享当前正在预览的内容”）。切换分享完整项目和分享当前预览后的功能同现有线上功能。
...
- **1. 发布前（未发布态）**
  - **弹框内容**：显示项目名称、当前状态、URL 链接和底部的发布按钮。
  - **当前状态**：从未发布成功过时显示灰色「未发布」胶囊；已成功发布过且当前在线时为绿色「已发布」；已成功发布过且用户已撤回（或链接失效）时为琥珀色「已下架」胶囊。
  - **项目名称（可编辑）**：
    - **默认与限制**：默认填充当前会话标题。长度限制 1-50 个字符（编辑时不可超 50 字符；非编辑状态超长则尾部省略号 `...` 截断）。
    - **格式支持**：支持任意语言、符号、数字组合。
  - **访问链接（可编辑）**：
    - **域名格式**：限制使用墨刀三级域名 `{slug}.modao.site`，不支持自定义独立域名。
    - **前缀规则**：仅限小写英文字母（a-z）、数字（0-9）和中划线（-），中划线不可在首尾。长度 3-30 个字符。
    - **交互态**：尚未产生过成功发布记录时，链接不可点；已有发布记录（含已撤回）后，hover 显示下划线，点击新标签打开预览页（演示环境为同源 Hash 预览地址 + 本地快照，正式上线可跳转真实 `{slug}.modao.site`）。
    - **编辑态**：点击编辑后图标变为"保存"和"取消"按钮，开始修改 `{slug}`。
    - **内容为空**：链接、项目名称任一内容为空时，发布按钮置灰不可点

- **2. 初次发布操作**
  - **按钮显示**：面板底部仅显示全宽的"**发布**"按钮。
  - **交互过程**：点击发布按钮后进入 Loading 状态（文本变为"正在发布..."） -> 发布成功后按钮显示为"撤回"与"更新"两个按钮,并在屏幕中上方弹toast提示“✅ 发布成功！网站已上线” ，发布面板新增选项"最近发布时间" （发布面板不消失）
  - **底层逻辑**：生成当前快照，初始版本号设为 V1。
  - **发布审核（分阶段）**：**当前演示/本期不接审核服务**时，仅展示「正在发布...」，加载完成后直接完成发布。接入线上审核后，在发布加载过程中进行内容审核，审核不通过则 Toast：内容不合规，请修改内容后重试。
  - **发布异常**：
    - 若域名被占用，弹toast提示"该链接已被占用"
    - 如遇到其他原因发布失败，弹toast提示：发布失败，请稍后重试
  
- **3. 发布后（已发布态）**
  - **弹框内容**：发布成功后，发布面板新增选项"最近发布时间"，面板底部按钮变为"撤回"与"更新"。
  - **当前状态**：显示绿色的"已发布"徽章。
  - **最近发布**：显示格式为"X 天前 · Vn"（n 从 1 开始递增）。

- **4. 更新操作**
  - **已发布 + 无变更**：若 `projectName` 及生成内容与上次发布快照完全一致，"**更新**"按钮呈禁用态灰色。用户只能进行撤回操作。
  - **已发布 + 有变更**："**更新**"按钮为激活态深色。
  - **交互过程**：点击更新按钮后进入 Loading 状态（"正在更新..."） -> 更新成功后按钮显示为"更新成功"，版本号 +1（如 V2），时间更新 -> 1s后更新成功按钮显示为禁用态，文案为“更新”
  - **更新审核（分阶段）**：与发布一致；**当前不接审核**时以「正在发布...」/「正在更新...」等价加载完成后直接成功。接入审核后，审核不通过则 Toast：内容不合规，请修改内容后重试。
  - **更新异常**：
    - 如遇到其他原因发布失败，弹toast提示：更新失败，请稍后重试
    - 链接、项目名称任一内容为空时，更新按钮置灰不可点

- **5. 撤回操作**
  - 发布后，撤回操作等同于下架
  - **按钮显示**：发布后恒定显示"**撤回**"按钮。
  - **交互过程**：点击撤回按钮 -> 弹框二次确认，单击确认 -> 弹框消失，发布面板进入撤回loading状态（正在撤回...）-> 撤回成功后弹toast提示“撤回成功”，撤回和发布按钮合并为正常态“发布”按钮，最近更新时间不消失，发布面板内容更新为“已下架”态，线上访问链接即刻失效(打开链接时显示作品已下架) 
  — **撤回异常**：如遇到其他原因撤回失败，弹toast提示：撤回失败，请稍后重试


- **6. 后台下架**
  - 作品因违规，在后台被下架时，作品状态显示为“已下架”和下架原因：内容不合规。
  - 因违规被后台下架后，整个对话中所有项目的发布弹窗中的发布按钮均置灰不可点击
  - 英文版：内容不合规/Content policy violation

   ![alt text](../assets/images/image-37.png)

- **7. 其他用户打开访问链接**
  - 应用发布后，用户点击访问链接，可正常查看应用内容，右下角显示生成同款和更多icon，同线上分享后的模块功能（生成同款可关闭，更多icon常显）

  ![alt text](../assets/images/image-38.png)

  - 应用下架后，点击访问链接显示作品已经下架
...
文档的功能同现有，仅针对顶部的分享、复制、下载、全屏、关闭的顺序和样式调整。

![alt text](../assets/images/image-2.png)

#### 3-2-6、积分消耗

积分消耗下拉菜单中增加一句话：AI生成应用 按实际消耗计费

![积分消耗与定价说明](../assets/images/image-40.png)
...
- 首页AI输入框
  - 入口：在工作台首页，选中墨刀原型 tab 后，下面的 AI 输入框内支持生成应用，根据用户输入的意图判断要生成的类型（输入框内不显示相关系统设置项）
  - 交互流程：用户单击发送按钮后，先创建原型文件，在原型中打开墨刀ai的页面，自动把输入的内容发送给ai开始生成
- 快速开始
  - 入口：在快速开始第二个位置增加“AI生成应用”入口
  - 交互流程：单击“AI生成应用”入口，在新tab打开墨刀ai，并默认在输入框内选中“AI生成应用”项，输入框内不带特定端型提示词前缀，由 AI 根据用户输入的意图自动判断并生成 Web 或 App
...
#### 3-3-2:墨刀原型编辑区
- 右侧栏快捷功能
  - 在编辑区未选中任何对象时，右侧栏去掉生成测试用例，增加“生成应用”，排在第一个，副标题为“生成一个完整的应用，内容为”
    - English: "Generate a complete app, content:"

   ![alt text](../assets/images/image-31.png)

  - 在编辑区选中编辑区的对象时，右侧栏去掉评审设计方案，增加“生成应用”，排在第一个，副标题为“根据选中的内容生成一个完整的应用”
    - English: "Generate a complete app based on the selection"

  ![alt text](../assets/images/image-29.png)

- 生成应用
  - 在编辑区右侧栏生成应用后，生成的应用自动插入到画布
    - 生成的应用插入到画布后，以ai组件的格式插入到画布（同现有的生成html），支持ai修改、切换页面和转为可编辑元素，暂不支持可视化编辑（置灰，hover时提示：react暂不支持，敬请期待）
    - ai修改、切换页面、转为可编辑元素的功能同现有能力
  - 在右侧AI栏的对话消息区，hover到生成的应用时，显示“插入到画布”入口
  - 在原型编辑区内，选中生成的文档、html、多页html和应用时，整个右侧的页面布局均与新版的墨刀ai右侧布局保持一致，“插入到画布”位于“导入墨刀原型”的位置
...
- **内容安全**：AI 生成的内容在生成后需经过后置敏感内容检测，触发安全红线时自动替换为风险占位符
- **域名防抢占**：发布域名在提交前进行数据库级别的唯一性锁定，防止并发抢注

### 5-3、兼容性要求

- **浏览器**：支持 Chrome 90+、Safari 14+、Firefox 90+、Edge（Chromium 版）；不承诺兼容 IE11
- **分辨率**：最小支持 **1440×900**，可降级到 1280×800（局部布局可能折叠），生成的应用界面支持自适应
...
**墨刀AI内**
  - 墨刀AI输入框内不支持生成应用的能力
  - 首页推荐tab下不显示 “AI生成应用” 二级tab，不显示“生成应用”一级 tab
  - 生成单页html和多页html后，右上角不显示发布按钮

**墨刀工作台**
  - 墨刀工作台原型tab下，输入框内不支持生成应用的能力。快速开始中不露出 AI生成应用 的入口
  
**墨刀原型内**
  - 原型编辑区内选中对象时，右侧栏不增加“生成应用”选项，保持现有不变
  - 原型编辑区内未选中对象时，右侧栏不增加“生成应用”选项，保持现有不变

### 6-2、海外版（本期同步做）

- UI 全面英文化，AI 生成的代码注释 and 文案默认为英文
