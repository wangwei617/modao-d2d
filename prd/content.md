...
- **权限**：同线上
- **模块内容**：采用左右分栏布局——左侧为历史对话区（历史导航 + 对话消息流 + 输入框），右侧为应用/文档展示区（预览/编辑/代码三大 Tab + 右侧目录 + 顶部工具栏）。

![对话工作台全貌](chat_workspace_full.png)

#### 2、左侧栏
- **左侧栏收起/展开**：左上角折叠图标控制历史导航区的展开状态，进入对话页后默认收起。左侧栏的功能和交互同线上
...
**地址栏操作区**：
  ...
  - *切换设备*
    - 单击切换设备，下拉显示设备列表，用户可以在PC端、移动端、Pad端之间切换。
    - 切换设备的默认项根据用户的生成的结果自动判断，不确定时默认选中 PC端；
      - 选中PC端时，下方的界面宽度/高度撑满显示区域
      - 选中移动端时，下方的界面限定在390px宽的显示区域，高度撑满显示区域
      - 选中Pad端时，下面的界面限定在800px宽的显示区域，高度撑满显示区域
    - 切换设备时，界面内容根据显示区域的宽度自适应显示。切换到移动端、pc端时，界面宽度之外的区域显示背景色，与界面之内的颜色做区分，类似下图，界面内与界面外要有边界区分
...
单页、多页保留现有的html格式，生成的应用默认采用react框架，代码显示区的展示样式同线上单页/多页的展示样式。
页面布局上，将下载功能从一级tab栏下移一层，且在选中目录中的某个页面时，顶栏显示对应的页面名称
...
**目录**：
  ...
  *目录结构与交互*
  - **展示逻辑**：应用代码模式下，左侧目录采用多层级树状结构（Tree View），深度与项目实际文件层级保持一致。
  - **文件夹交互**：
  ...
  - **文件交互**：
    ...
    - **文件图标枚举**（UI设计需覆盖以下类型）：
      - **React 组件**：`.tsx`, `.jsx`（建议使用 React 原子图标）
      - **脚本/逻辑**：`.ts`, `.js`, `.mjs`, `.cjs`（对应的语言标识）
      - **样式表**：`.css`, `.less`, `.scss`（# 标识或样式图标）
      - **标记语言**：`.html`（尖括号图标）
      - **配置文件**：`.json`, `.yaml`, `.yml`, `.toml`（设置图标）
      - **特殊配置**：`package.json`, `tsconfig.json`, `vite.config.ts`, `tailwind.config.js`, `.eslintrc`, `.gitignore`（特定文件名或特定品牌图标）
      - **多媒体/资源**：`.svg`, `.png`, `.jpg`, `.jpeg`, `.gif`, `.webp`, `.ico`（图片图标）
      - **说明文档**：`.md`（M↓ 标识）
  - **视觉样式与极致缩进处理**：
    - 每一级子目录向右缩进固定距离（建议 12px-16px），增强层级感；
    - **溢出处理策略**：当层级过多导致缩进后的文件名显示空间不足时：
      1. **横向滚动**：目录区域下方出现水平滚动条（默认隐藏，滚动时显示），确保用户可查看完整层级；
      2. **文字截断**：单个文件名超长时，末尾显示省略号，hover 时通过 Tooltip 展示全名。
    - 鼠标 hover 列表项时，单行显示浅色背景，提升视觉反馈。


##### 4）截图优化
hover到截图优化时显示tooltip：截图优化
- **入口位置**：预览 Tab 和编辑 Tab 的内容区左上角均固定展示 📷 截图优化图标按钮。
- **截取预览区**：通过 `html2canvas` 将沙盒 iframe 内容绘制为 Canvas，导出为 PNG 格式图像。注意：若跨域限制导致无法截取，Toast 提示"无法截取当前内容，请尝试直接框选屏幕区域"。
- **截取全屏**：调用 `getDisplayMedia()` API，用户需授权共享屏幕；不支持该 API 时，此选项直接隐藏。
- **标注编辑**：截取成功后，截图进入标注模式，工具栏提供画笔、矩形框、文字、颜色选取、撤销（`Cmd+Z`）。截图区域若超屏，内部出现滚动条。
- **添加到对话**：点击"添加到对话"，截图合并标注层后以缩略图附件形式插入左侧输入框（已有 3 张时提示"已达图片上限"）。

##### 5）导出至墨刀
- 单页/多页的导出至墨刀功能同线上
- 应用的导出至墨刀功能，支持将应用的按照一个个单页面的方式导入到原型编辑区
  - 导入过程的交互和导入后在编辑区的显示格式同单页/多页的导入
  - 导入后，各个页面按照顺序从左向右展示，间距10px

##### 6）分享
- **分享项目**：点击顶部"↑ 分享"按钮，弹出分享设置弹窗；支持"分享整个项目"（全部文件）和"分享单个文件"两种粒度。
- **权限控制**：可选"公开访问（任何人可查看）"或"仅链接访问（知道链接者可看）"，通过分享链接进入的访客只有预览权限，所有编辑工具栏不可见。
- **链接复制**：生成唯一分享链接后，一键复制到剪贴板，Toast 提示"链接已复制"，持续 2s。

##### 7）发布
- **发布入口**：顶部右侧醒目的黑色"**发布**"按钮，随时可点击发起发布流程。
- **发布配置弹窗**：居中展开 Modal（`scale(0.95→1) 0.2s ease-out`），底层覆盖半透明黑色遮罩；包含以下表单项：
  - **项目名称**：默认填充会话标题，支持修改（1-50字）。
  - **二级域名**：格式规则为仅英文字母（a-z）、数字（0-9）、横线（-），长度 3-20 位；失焦（OnBlur）立即触发后端域名占用检测，冲突时红字报错"域名已被占用"；非法字符实时过滤，按键不生效。
  - **访问权限**：三选一枚举——公开（任何人）、仅链接访问、私密（仅创建者）。
- **部署流**：点击"确认发布"后，按钮进入 Loading 转圈；后台执行代码打包 → CDN 上传 → 域名绑定（超时定义 > 20s 则判定失败）；成功后弹窗切换为"发布成功"态，显示公网访问链接，提供"复制链接"和"立即访问"按钮。
- **重试机制**：发布失败时弹窗内展示"重试"按钮（不自动重试）；构建超时提示"构建环境繁忙，稍后重试"；域名抢占冲突提示"域名已被抢先使用，请重新选择"。
- **弹窗关闭保护**：修改域名后若点击关闭遮罩或按 `Esc`，弹出二次确认"确认放弃当前修改？"。
