...
import { ChevronDown, CircleHelp, PanelLeft, Sparkles, Check, Copy, Download, RotateCw, Smartphone, Tablet, Monitor, Share, Share2, Maximize2, Edit3, FileText, CheckCircle2, Paperclip, ArrowUp, X, Globe, Lock, Settings, BarChart3, Image, RefreshCw, MonitorSmartphone, Code2, Palette, AlertCircle, PackageX } from 'lucide-react';
...
import { getRequirementConfirmationByLabel, type RequirementConfirmationRecord } from '@/lib/requirementConfirmations';
...
function buildRequirementReplyTemplate(record: RequirementConfirmationRecord) {
    return record.questions
        .map((question, index) => `${index + 1}. ${question.label}：`)
        .join('\n');
}
    ...
    return (
        <div className="space-y-4 mb-8">
            <div className="flex gap-2.5">
                <div className="mt-0.5 w-6 h-6 rounded-full bg-gradient-to-tr from-indigo-500 to-violet-500 shrink-0 text-white flex items-center justify-center shadow-sm">
                    <Sparkles size={12} fill="white" />
                </div>
                <div className="flex-1 space-y-3">
                    <div className="bg-white text-slate-700 px-4 py-3 rounded-2xl rounded-tl-sm border border-slate-100 max-w-[100%]">
                        <p className="text-[13px] leading-relaxed">请确认以下信息，以便我更好地理解您的需求。</p>
                    </div>

                    <div className="rounded-[24px] bg-[#F5F5FF] border border-[#E8E9FF] p-4">
                        <div className="text-[14px] font-semibold text-slate-800">补充内容</div>

                        <div className="mt-4 rounded-[18px] border border-slate-200 bg-white px-4 py-4">
                            <div className="flex items-center justify-between gap-3">
                                <div className="text-[14px] font-semibold text-slate-800">总览并确认</div>
                                <div className="text-[12px] font-semibold text-slate-500">
                                    {record.questions.length} / {record.questions.length}
                                </div>
                            </div>
                            ...
                            <div className="mt-4 space-y-5">
                                {record.questions.map((question) => (
                                    <div key={question.id}>
                                        <div className="text-[13px] font-semibold text-slate-800">{question.label}？</div>
                                        <div className="mt-1 flex items-center gap-2 text-[12px] text-[#7C80F6]">
                                            <span className="h-1.5 w-1.5 rounded-full bg-[#7C80F6] shrink-0" />
                                            <span className="text-slate-500">{question.examples[0]}</span>
                                        </div>
                                    </div>
                                ))}
                            </div>
                        ...
                        <div className="mt-4 flex items-center justify-between gap-3">
                            <div className="flex items-center gap-2 text-[12px] text-slate-500">
                                <CircleHelp size={14} className="text-slate-400 shrink-0" strokeWidth={2.2} />
                                <span>未选择或未填写的内容，将由 AI 自动处理</span>
                            </div>
                            <Button
                                type="button"
                                onClick={onOpen}
                                className="h-11 rounded-xl bg-[#6366F1] px-8 text-[14px] font-semibold text-white hover:bg-[#575AE6]"
                            >
                                确认
                            </Button>
                        </div>
...
export function ChatPage() {
    const { userPrompt, setUserPrompt, sidebarCollapsed, setSidebarCollapsed, setViewMode, setActiveNav, setAppSurface, activeChatLabel } = useSidebarContext();
    ...
    const activeRequirementConfirmation = getRequirementConfirmationByLabel(activeChatLabel);
    ...
    const handleConfirmRequirement = () => {
        if (!activeRequirementConfirmation) return;
        setInputMessage(buildRequirementReplyTemplate(activeRequirementConfirmation));
    ...
    return (
        ...
        <>
                ...
                >
                        ...
                        {sidebarCollapsed && (
                            <>
                                ...
                                <div className="relative group/newchat shrink-0">
                                    <button
                                        onClick={() => { setViewMode('home'); setAppSurface('ai-home'); setActiveNav('modao-ai'); setUserPrompt(''); }}
                    ...
                    <ScrollArea className="flex-1 px-4">
                        <div className="space-y-4 pb-6 mt-4">
                            {activeRequirementConfirmation && (
                                <RequirementConfirmationCard
                                    record={activeRequirementConfirmation}
                                    onOpen={() => setShowRequirementDialog(true)}
                                />
                            )}

                            {/* Empty Default State */}
                            {sessions.length === 0 && !activeRequirementConfirmation && (
                                ...
                                <div className="flex items-end gap-3">
                                    <div className="flex-1 min-h-[44px] flex flex-col justify-center">
                                        <textarea
                                            ...
                                            placeholder={activeRequirementConfirmation ? '补充待确认需求，AI 会继续往下处理...' : 'Ask a question or make a change...'}
            ...
            <Dialog open={showRequirementDialog} onOpenChange={setShowRequirementDialog}>
                <DialogContent className="max-w-[640px] gap-0 rounded-[28px] border border-[#F1E2BE] bg-[#FFFDF7] p-0 shadow-[0_28px_80px_-32px_rgba(15,23,42,0.35)]">
                    {activeRequirementConfirmation && (
                        <div className="overflow-hidden rounded-[28px]">
                            <div className="border-b border-[#F5E7C4] bg-[linear-gradient(135deg,#FFF9EC_0%,#FFF3D8_100%)] px-7 py-6">
                                <div className="flex items-start gap-4">
                                    <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-[#D97706] text-white shadow-[0_16px_32px_-20px_rgba(217,119,6,0.8)]">
                                        <CircleHelp size={22} strokeWidth={2.2} />
                                    </div>
                                    <DialogHeader className="space-y-2 text-left">
                                        <div className="inline-flex w-fit items-center gap-1.5 rounded-full border border-[#F3D08E] bg-white/90 px-2.5 py-1 text-[11px] font-semibold text-[#9A6700]">
                                            <span className="h-1.5 w-1.5 rounded-full bg-[#F59E0B]" />
                                            {activeRequirementConfirmation.badgeText}
                                        </div>
                                        <DialogTitle className="text-[22px] font-semibold tracking-tight text-slate-900">
                                            {activeRequirementConfirmation.title}
                                        </DialogTitle>
                                        <DialogDescription className="text-[14px] leading-6 text-slate-600">
                                            {activeRequirementConfirmation.summary}
                                        </DialogDescription>
                                    </DialogHeader>
                                </div>
                            </div>
                            ...
                            <div className="px-7 py-6">
                                <div className="grid gap-6 md:grid-cols-[1.05fr_1.35fr]">
                                    <div className="rounded-3xl border border-[#F5E7C4] bg-white p-5">
                                        <div className="text-[13px] font-semibold text-slate-800">{activeRequirementConfirmation.missingInfoLabel}</div>
                                        <div className="mt-4 space-y-3">
                                            {activeRequirementConfirmation.missingInfo.map((item) => (
                                                <div key={item} className="flex items-start gap-2.5 text-[13px] leading-6 text-slate-600">
                                                    <span className="mt-2 h-1.5 w-1.5 rounded-full bg-[#F59E0B] shrink-0" />
                                                    <span>{item}</span>
                                                </div>
                                            ))}
                                        </div>
                                    ...
                                    <div className="space-y-3">
                                        {activeRequirementConfirmation.questions.map((question, index) => (
                                            <div key={question.id} className="rounded-3xl border border-[#F5E7C4] bg-white p-5">
                                                <div className="flex items-start gap-3">
                                                    <div className="flex h-7 w-7 items-center justify-center rounded-full bg-[#FFF3D8] text-[12px] font-semibold text-[#B45309] shrink-0">
                                                        {index + 1}
                                                    </div>
                                                    <div className="min-w-0">
                                                        <div className="text-[14px] font-semibold text-slate-800">{question.label}</div>
                                                        <div className="mt-1 text-[12px] leading-5 text-slate-500">{question.helper}</div>
                                                        <div className="mt-3 flex flex-wrap gap-2">
                                                            {question.examples.map((example) => (
                                                                <span key={example} className="rounded-full bg-[#FFF8EB] px-3 py-1 text-[12px] text-[#9A6700]">
                                                                    {example}
                                                                </span>
                                                            ))}
                                                        </div>
                                ...
                                <div className="mt-6 flex items-center justify-between gap-4 rounded-3xl border border-dashed border-[#F3D08E] bg-[#FFF8EB] px-5 py-4">
                                    <div>
                                        <div className="text-[13px] font-semibold text-slate-800">确认后会发生什么</div>
                                        <div className="mt-1 text-[12px] leading-5 text-slate-500">{activeRequirementConfirmation.responseExpectation}</div>
                                    </div>
                                    <div className="shrink-0 text-[12px] font-medium text-[#B7791F]">{activeRequirementConfirmation.updatedAt}</div>
                                </div>
                                ...
                                <div className="mt-6 flex items-center justify-end gap-3">
                                    <Button
                                        type="button"
                                        variant="ghost"
                                        className="h-11 rounded-xl px-4 text-slate-600 hover:bg-slate-100"
                                    ...
                                    <Button
                                        type="button"
                                        className="h-11 rounded-xl bg-[#D97706] px-5 text-white hover:bg-[#B45309]"
