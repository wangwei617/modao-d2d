...
- **1. 发布前（未发布态）**
  - **弹框内容**：显示项目名称、发布状态、URL 链接和底部的发布按钮。
  - **当前状态**：显示灰色的"未发布"胶囊。
  - **项目名称（可编辑）**：
...
- **2. 初次发布操作**
  ...
  - **交互过程**：点击后进入 Loading 状态（文本变为"正在发布..."） -> 发布成功后按钮显示为发布成功 -> 1s后发布面板自动关闭
  - **底层逻辑**：生成当前快照，初始版本号设为 V1。

- **3. 发布后（已发布态）**
  - **弹框内容**：发布成功后再次单击发布按钮，弹出发布面板。面板新增选项"最近发布"，底部按钮变为"撤回"与"更新"。
  - **当前状态**：显示绿色的"已发布"徽章。
...
- **4. 更新操作**
  ...
  - **已发布 + 有变更**："**更新**"按钮为激活态深色。
  - **交互过程**：点击更新按钮后进入 Loading 状态（"正在更新..."） -> 更新成功后按钮显示为"更新成功"，版本号 +1（如 V2），最近发布的时间更新 -> 1s后弹框自动消失（再次打开时“更新”按钮禁用）

- **5. 撤回操作**
  - **按钮显示**：发布后恒定显示"**撤回**"按钮。
  - **交互过程**：点击撤回按钮 -> 弹框二次确认，确认撤回 -> 进入撤回loading状态（正在撤回...）-> 撤回成功后弹toast提示“撤回成功”，线上访问链接即刻失效(打开链接时显示404) -> 再次打开发布面板时（发布面板内容更新为未发布态）。
