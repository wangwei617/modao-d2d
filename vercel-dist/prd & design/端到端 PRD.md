...
- **已确认选框的交互**：
  ...
  - **Hover 态**：鼠标悬停在数字徽章上时，展开选框蓝色边框 + 下方文本弹出卡片；卡片底部显示「删除」按钮，点击后移除该标注。
- **选框拖动**：鼠标悬停在未确认选框的框体中心时，光标切换为抓手（`grab`）；按住后可自由拖动整个选框位置。
...
##### 5）导出至墨刀
- 单页/多页的导出至墨刀功能同线上
- 应用的导出至墨刀功能，支持将应用的页面按照一个个单页面的方式导入到原型编辑区
  - 导入过程的交互和导入后在编辑区的显示格式同单页/多页的导入
  - 导入后，各个页面按照页面地址栏的上下顺序在画布中从左向右展示，间距10px

##### 6）分享
- **分享入口**：点击顶部"↑ 分享"按钮，在按钮下方以 Popover 形式弹出分享设置面板（`fade-in + slide-in-from-top`），点击面板外部自动关闭。
- **分享粒度**：面板顶部提供两个标签页切换——"分享项目"（全部文件）和"分享文件"（当前单文件），默认选中"分享项目"；选中态为白色背景+蓝色文字，未选中态为透明背景+灰色文字，整体容器为浅灰底圆角胶囊。
- **权限控制**：可选"仅限自己（仅自己可见）"或"公开（获得链接的人可访问）"，默认选中"公开"，选中项高亮显示勾选图标（蓝色）；通过分享链接进入的访客只有预览权限，编辑工具栏不可见。
- **回放模式**：面板内提供回放模式开关，默认开启；开启后访客以幻灯片播放方式浏览原型。
- **复制链接**：点击紫色"复制链接"按钮，将分享链接写入剪贴板，Toast 提示"链接已复制"持续 2s。
- **发布项目**：仅在"分享项目"标签下显示蓝色"发布项目"按钮，点击触发发布流程（与顶部"发布"按钮行为一致）。

##### 7）发布
- **发布入口**：顶部右侧"**发布**"按钮，点击在按钮下方以 Popover 形式弹出发布面板（`fade-in + slide-in-from-top`），点击面板外部自动关闭。
- **发布面板结构**：宽 420px，含标题栏（"发布网站" + 关闭 ×）、表单区、底部操作栏三部分。
- **表单字段**：
  - **项目名称**：默认填充当前会话标题，支持修改（1-50 字）。
  - **当前状态**：仅在已发布后显示绿色"已发布"徽章。
  - **最近发布**：仅在已发布后显示，格式为"X 天前 · Vn"（n 为版本号，从 V1 开始递增）。
  - **访问链接**：显示当前域名（格式 `{slug}.modao.site`），后跟三个操作图标：复制链接（点击变绿色勾 2s）、随机刷新域名、编辑自定义域名；域名占用时红字报错"该链接已被占用"。
- **底部操作按钮**：
  - **初次发布（未发布态）**：仅显示全宽深色"发布"按钮；点击后按钮进入 Loading → 1.5s 后切换为绿色"发布成功"，同时关闭面板，`isPublished` 置为 true，版本号置 V1。
  - **已发布 + 无变更**：显示"取消发布"和禁用态灰色"更新"按钮（`cursor-not-allowed`）；变更判定依据：`projectName` 与发布时快照不一致。
  - **已发布 + 有变更**：显示"取消发布"和激活态深色"更新"按钮；点击"更新"后同样进入 Loading → 绿色"更新成功"，版本号 +1，快照更新。
  - **取消发布**：点击后 `isPublished` 置 false，版本号清零，面板关闭。
