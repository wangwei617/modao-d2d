...
1. **首页输入工作区**：增加生成应用一级类别&二级类别、推荐下增加生成应用入口、选中生成应用时输入框调整
2. **生成工作台**：顶部栏功能布局调整、预览应用、可视化局部编辑、代码查看器
3. **截图优化工具**：截图入口、标注编辑、插入对话
4. **发布与分发中心**：域名修改、一键部署
5. **问询/追问模式**：问询、追问提示词微调
6. **工作台首页**：墨刀原型卡片下的快速开始模块，增加生成应用入口，墨刀ai原型输入框支持生成应用
...
- 生成应用后，单击导出至墨刀，下拉显示菜单，分别为导出至墨刀设计和导出至墨刀原型
  ...
  - 导出至墨刀原型（置灰，本期不做）：
    - 支持将 React 框架生成的应用，将整个应用作为一个ai组件导入到原型编辑区，功能同多页面导入至墨刀
...
- **1. 发布前（未发布态）**
  ...
  - **访问链接（可编辑）**：
    ...
    - **交互态**：未发布时，hover到链接无反应，链接不可点（发布后，hover到链接时，链接下方显示下划线，点击链接新tab打开显示生成的内容）。
...
- **2. 初次发布操作**
  ...
  - **发布审核**：在单击发布按钮后，显示正在发布中，这个过程中需要对用户发布的内容（包括项目名称、域名和生成的内容）进行在线审核，审核OK后才会继续完成发布，若审核不通过，则弹toast提示：内容不合规，请修改内容后重试
  - **发布异常**：
...
- **4. 更新操作**
  ...
  - **更新审核**：在单击更新按钮后，显示“正在更新中”，这个过程中需要对用户更新的内容（包括项目名称、域名和生成的内容）进行在线审核，审核OK后才会继续完成发布，若审核不通过，则弹toast提示：内容不合规，请修改内容后重试
  - **更新异常**：
...
- **5. 撤回操作**
  - 发布后，撤回操作等同于下架
  - **按钮显示**：发布后恒定显示"**撤回**"按钮。
  - **交互过程**：点击撤回按钮 -> 弹框二次确认，单击确认 -> 弹框消失，发布面板进入撤回loading状态（正在撤回...）-> 撤回成功后弹toast提示“撤回成功”，撤回和发布按钮合并为正常态“发布”按钮，最近更新时间不消失，发布面板内容更新为未发布态，线上访问链接即刻失效(打开链接时显示404) 
...
#### 3-3-2:墨刀原型编辑区
- 在原型编辑区内右侧栏支持生成应用
  - 未选中任何对象时，去掉生成测试用例，增加“生成应用”，排在第一个，副标题为“生成一个完整的应用，内容为”
    - English: "Generate a complete app, content:"

   ![alt text](../assets/images/image-31.png)

  - 选中编辑区的对象时，去掉评审设计方案，增加“生成应用”，排在第一个，副标题为“根据选中的内容生成一个完整的应用”
    - English: "Generate a complete app based on the selection"

  ![alt text](../assets/images/image-29.png)

- 在原型编辑区内的墨刀AI中生成应用后
  - 在右侧AI栏的对话消息区，hover到生成的应用时，不显示“插入到画布”入口
  - 在原型编辑区内，选中生成的文档、html、多页html时，整个右侧的页面布局均与新版的墨刀ai右侧布局保持一致，“插入到画布”位于“导入墨刀原型”的位置

  ![alt text](../assets/images/image-33.png)

  - 在原型编辑区内，选中生成的应用时，整个右侧的页面布局均与新版的墨刀ai右侧布局保持一致，不显示“插入到画布”
