...
**地址栏操作区**：
  ...
  - *切换设备*
    ...
    - 切换设备的默认项根据用户的生成的结果自动判断，不确定时默认选中 PC端；
      ...
      - 选中Pad端时，下方生成的界面最大区域为820px宽，1180px高；右侧栏区域小于界面的最大显示区域时，自适应撑满对应的宽/高
      - 选中移动端时，下方生成的界面最大区域为393px宽，852px高；右侧栏区域小于界面的最大显示区域时，自适应撑满对应的宽/高
...
**标题**：
  - 选中生成的单页、多页、应用时，在目录右侧区域显示当前所展示代码的页面标题
...
- **1. 发布前（未发布态）**
  ...
  - **访问链接（可编辑）**：
    ...
    - **校验规则**：光标移出输入框时进行校验，若含有敏感词红字报错"该链接包含违规字符"；若域名被占用红字报错"该链接已被占用"。
