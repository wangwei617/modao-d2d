...
- **1. 发布前（未发布态）**
  ...
  - **项目名称（可编辑）**：
    ...
    - **格式支持**：支持任意语言、符号、数字组合。
    - **校验规则**：接入敏感词过滤，保存时若违规，飘红提示"包含敏感词汇，请修改"。
  - **访问链接（可编辑）**：
    - **域名格式**：限制使用墨刀三级域名 `{slug}.modao.site`，不支持自定义独立域名。
    - **前缀规则**：仅限小写英文字母（a-z）、数字（0-9）和中划线（-），中划线不可在首尾。长度 3-30 个字符。
    - **交互态**：点击文本直接复制。尾部带“复制（点击变绿勾 2s）”和“编辑”图标。
    - **编辑态**：点击编辑后图标变为"保存"和"取消"按钮，开始修改 `{slug}`。
    - **校验规则**：含有敏感词红字报错"该链接包含违规字符"；域名被占用红字报错"该链接已被占用"。

- **2. 初次发布操作**
  - **按钮显示**：面板底部仅显示全宽深色的"**发布**"按钮。
  - **交互过程**：点击后进入 Loading 状态（文本变为"正在发布..."） -> 1.5s 后切换为绿色"发布成功" -> 面板自动关闭。
  - **底层逻辑**：生成当前快照，`isPublished` 置为 true，初始版本号设为 V1。

- **3. 发布后（已发布态）**
  - **弹框内容**：再次点开发布面板，新增显示"最近发布时间"，底部按钮变为"撤回"与"更新"。
  - **当前状态**：显示绿色的"已发布"徽章。
  - **最近发布**：显示格式为"X 天前 · Vn"（n 从 1 开始递增）。

- **4. 更新操作**
  - **已发布 + 无变更**：若 `projectName` 及生成内容与上次发布快照完全一致，"**更新**"按钮呈禁用态灰色（`cursor-not-allowed`）。用户只能进行撤回操作。
  - **已发布 + 有变更**："**更新**"按钮为激活态深色。
  - **交互过程**：点击后进入 Loading 状态（"正在发布..."） -> 完成后显示绿色"更新成功" -> 版本号 +1（如 V2），覆盖刷新发布快照。

- **5. 撤回操作**
  - **按钮显示**：发布后恒定显示"**撤回**"按钮。
  - **交互过程**：点击后 `isPublished` 置为 false，版本号清零，面板自动关闭，线上访问链接即刻失效。
