...
- **墨刀 D2D 工作区**
  - **首页**
    ...
    - AI能力（分类展示，选中后输入框显示对应标签或新tab打开）：
      - **推荐**：生成Web应用、AI生成原型、图片转原型、需求文档PRD、HTML转原型、生成图片
      - **生成应用**：生成Web应用、生成App应用
...
- 单页/多页的导出至墨刀功能同线上。
- 生产应用后，单击导出至墨刀，下拉显示菜单，分别为导出至墨刀原型和导出至墨刀设计
  - 导出至墨刀原型：
...
- **1. 发布前（未发布态）**
  ...
  - **项目名称（可编辑）**：
    ...
    - **格式支持**：支持任意语言、符号、数字组合。
    - **校验规则**：接入敏感词过滤，光标移出输入框时进行校验，若违规，输入框下方飘红提示"包含敏感词汇，请修改"。
  - **访问链接（可编辑）**：
    ...
    - **校验规则**：
      - 光标移出输入框时进行校验，若含有敏感词红字报错"该链接包含违规字符"；若域名被占用红字报错"该链接已被占用"。
    - **报错提示交互**
      - 输入框内容提示报错时，输入框下方的间距增大，保证报错提示可以正常显示
      - 输入框内容变更时，对应的报错提示消失
...
- **2. 初次发布操作**
  ...
  - **发布审核**：在单击发布按钮后，显示正在发布中，这个过程中需要对用户生成的内容进行在线审核，审核OK后才会继续完成发布，若审核不通过，则弹toast提示：内容不合规，请修改内容后重试
  — **发布异常**：如遇到其他原因发布失败，弹toast提示：发布失败，请稍后重试
...
- **4. 更新操作**
  ...
  - **更新审核**：在单击更新按钮后，显示“正在更新中”，这个过程中需要对用户生成的内容进行在线审核，审核OK后才会继续完成发布，若审核不通过，则弹toast提示：内容不合规，请修改内容后重试
  — **更新异常**：如遇到其他原因发布失败，弹toast提示：更新失败，请稍后重试
...
- 在工作台首页，选中墨刀原型tab后，下面的AI输入框内支持生成应用，根据用户输入的意图判断（输入框内不显示相关系统设置项）
- 在快速开始入口内增加生成web应用、生成App应用入口
  - 单击生产Web应用入口，当前页新建原型并进入编辑区，全屏显示墨刀ai页面，输入框内带提示词：生成Web应用，内容为
    - English: “Generate a Web App, content:”
  - 单击生产App应用入口，当前页新建原型并进入编辑区，全屏显示墨刀ai页面，输入框内带提示词：生成App应用，内容为
    - English: “Generate an App, content:”
...
### 6-1、私有化
- 私有化项目不支持生成应用的相关功能
  - 墨刀AI输入框内不支持生成应用的能力，首页推荐tab下不显示生成web应用的入口，不显示生成应用以及tab
  - 墨刀工作台原型tab下，输入框内不支持生成应用的能力。快速开始中不露出生成web应用和生成app应用的入口
