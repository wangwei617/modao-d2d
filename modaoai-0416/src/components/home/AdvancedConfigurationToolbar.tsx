...
export function AdvancedConfigurationToolbar({ showDesignSystemSelector = true, inline = false }: AdvancedConfigurationToolbarProps) {
    ...
    return (
        <div className={cn("flex items-center gap-2 animate-in fade-in slide-in-from-top-1 duration-300", !inline && "mt-4 pt-4 border-t border-gray-100")}>
            ...
            {/* Inline: merge Design System + Theme into a single settings entry */}
            {inline ? (
                <div className="relative" ref={moreRef}>
                    <button
                        ...
                        className={cn(
                            'flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-xs transition-colors bg-white shadow-sm',
                            showMoreDropdown
                                ? 'border-indigo-200 bg-indigo-50/80 text-indigo-600'
                                : 'border-gray-200 text-gray-600 hover:bg-gray-50',
                        )}
                    >
                        <MoreHorizontal size={13} className={showMoreDropdown ? 'text-indigo-500' : 'text-gray-500'} />
                    </button>
                    {showMoreDropdown && (
