...
- **功能概述**：提供对当前生成内容的快速抓取与标注反馈能力，解决文字指令针对视觉细节描述困难的问题。
- **全屏模式交互**：
  - **入口**：点击预览/编辑 Tab 内容区左上角的 📷 **截图优化**图标，进入全屏工作模式。
  - **显示页面**：全屏内展示的页面内容必须与用户在 Tab 中当前停留的页面保持一致。
  - **显示逻辑**：展示范围同「新 Tab 打开」，采用自适应（Fluid）布局填充浏览器视口，不保留 App/Pad/Web 的设备外壳限制，以确保最大标注操作空间。
- **全屏工具栏 (Top Toolbar)**：
  - 固定于屏幕顶部中央，包含以下模块：
    - **标注工具组**：画笔、矩形框、文字、颜色选择、撤销（`Cmd+Z`）。
    - **添加到对话**：合并标注层并导出为 PNG，作为附件插入左侧输入框（上限 3 张）。点击后自动退出全屏模式。
    - **关闭/退出**：点击关闭按钮或按 `Esc` 键退出，不保存当前标注。
- **技术规格与异常处理**：
  - **截取预览区**：通过 `html2canvas` 抓取。若遇跨域限制导致 iframe 白屏，Toast 提示："无法截取当前内容，请尝试直接框选屏幕区域或手动粘贴图片"。
  - **长图支持**：页面内容若超过屏幕高度，全屏容器内应支持纵向滚动以查看完整页面并进行局部标注。
